...
class SaltyAlert {
    ...
    createBackdrop () : HTMLDivElement {
        ...
        backdrop.classList.add('saltyalert-backdrop');
        return backdrop;
    }
    ...
    createModalCloseIcon (alertWrapper: HTMLElement, modal: HTMLElement) : HTMLSpanElement {
        ...
        close.classList.add('saal-close');
    ...
    createSvgIcon (iconName: string) : HTMLSpanElement {
        let _ = document.createElement('div');
        _.innerHTML = this.getSvg(iconName);

        const svg = document.createElement('span');
        svg.append(_.firstChild!);
        svg.classList.add('saal-icon');
    ...
    async modal(props: ModalProperties) {
        const wrapper = this.createBackdrop();
        const saltyalert = document.createElement('div');
        ...
        // Creating close icon
        const close = this.createModalCloseIcon(wrapper, saltyalert);
        // Creating icon element
        const svg = this.createSvgIcon(props.icon);
        ...
        saltyalert.appendChild(close);
        saltyalert.appendChild(svg);
        saltyalert.appendChild(textContent);
        saltyalert.classList.add('saltyalert', 'modal');

        saltyalert.style.width = '300px';
        saltyalert.style.height = '320px';
        ...
        if (props.showConfirmButton || props.showCancelButton) {
            ...
            saltyalert.appendChild(actions);
        }

        this.body.appendChild(wrapper);
        this.body.appendChild(saltyalert);
